# nanobot 项目学习指南

## 项目概览

**nanobot** 是一个超轻量级（~4,000 行核心代码）的个人 AI 助手框架。

### 核心特点
- 🪶 超轻量：仅 4,000 行核心代码（相比 Clawdbot 的 430,000 行减少 99%）
- ⚡️ 高性能：最小化设计意味着快速启动和低资源消耗
- 🔬 易研究：代码清晰易读，易于理解和修改
- 🔌 多平台：支持 Telegram、Discord、WhatsApp、Feishu、Slack 等多个聊天平台
- 🤖 多模型：支持 OpenRouter、Anthropic、OpenAI、DeepSeek 等多个 LLM 提供商

---

## 🏗️ 整体架构

### 架构层次（自顶向下）

```
┌─────────────────────────────────────────────────────────┐
│                    用户交互层                            │
│         (CLI / Telegram / Discord / ...)                │
└────────────────────┬────────────────────────────────────┘
                     │
┌────────────────────▼────────────────────────────────────┐
│                   消息路由层                              │
│              MessageBus (消息队列)                        │
│   ┌─────────────────────────────────────────────────┐   │
│   │  Inbound Queue  │  Outbound Queue               │   │
│   └─────────────────────────────────────────────────┘   │
└────────────────────┬────────────────────────────────────┘
                     │
┌────────────────────▼────────────────────────────────────┐
│                  核心处理层                               │
│              AgentLoop (代理循环)                         │
│   ┌─────────────────────────────────────────────────┐   │
│   │  1. 接收消息                                     │   │
│   │  2. 构建上下文 (ContextBuilder)                  │   │
│   │  3. 调用 LLM                                     │   │
│   │  4. 执行工具 (ToolRegistry)                      │   │
│   │  5. 返回响应                                     │   │
│   └─────────────────────────────────────────────────┘   │
└────────────────────┬────────────────────────────────────┘
                     │
         ┌───────────┼───────────┐
         │           │           │
    ┌────▼───┐  ┌───▼────┐  ┌──▼─────┐
    │ LLM    │  │ Tools  │  │ Memory │
    │ Provider│  │Registry│  │ Store  │
    └────────┘  └────────┘  └────────┘
```

### 核心模块关系图

```
nanobot/
├── agent/              # 🧠 核心代理逻辑
│   ├── loop.py         #    AgentLoop - 主处理循环
│   ├── context.py      #    ContextBuilder - 构建提示词
│   ├── memory.py       #    MemoryStore - 记忆存储
│   ├── skills.py       #    SkillsLoader - 技能加载
│   ├── subagent.py     #    SubagentManager - 子代理管理
│   └── tools/          #    工具集合
├── channels/           # 📱 聊天渠道适配器
│   ├── telegram.py     #    Telegram 适配器
│   ├── discord.py      #    Discord 适配器
│   ├── whatsapp.py     #    WhatsApp 适配器
│   ├── feishu.py       #    飞书适配器
│   ├── slack.py        #    Slack 适配器
│   └── manager.py      #    渠道管理器
├── bus/                # 🚌 消息总线
│   ├── queue.py        #    MessageBus - 消息队列
│   └── events.py       #    事件定义
├── providers/          # 🤖 LLM 提供商
│   ├── registry.py     #    提供商注册表
│   ├── litellm_provider.py
│   ├── custom_provider.py
│   └── openai_codex_provider.py
├── session/            # 💬 会话管理
├── config/             # ⚙️ 配置管理
├── cron/               # ⏰ 定时任务
├── heartbeat/          # 💓 心跳机制
├── skills/             # 🎯 内置技能
└── cli/                # 🖥️ 命令行接口
```

---

## 📊 数据流分析

### 完整消息处理流程

```
用户发送消息
    │
    ▼
┌──────────────┐
│ Channel 适配器 │
│ (接收平台消息) │
└──────┬───────┘
       │
       ▼
┌──────────────┐
│ 转换为        │
│ InboundMessage│
└──────┬───────┘
       │
       ▼
┌──────────────┐
│ MessageBus   │
│ Inbound Queue│
└──────┬───────┘
       │
       ▼
┌──────────────┐
│ AgentLoop    │
│ _process_    │
│   message()  │
└──────┬───────┘
       │
       ├─▶ Session 获取/创建
       │
       ├─▶ 构建上下文 (history + skills + system)
       │
       ├─▶ 调用 LLM
       │
       ├─▶ 执行工具调用 (如果有)
       │   │
       │   └─▶ 循环直到完成
       │
       ▼
┌──────────────┐
│ 保存到        │
│ Session      │
└──────┬───────┘
       │
       ▼
┌──────────────┐
│ 转换为        │
│ OutboundMsg  │
└──────┬───────┘
       │
       ▼
┌──────────────┐
│ MessageBus   │
│Outbound Queue│
└──────┬───────┘
       │
       ▼
┌──────────────┐
│ Channel 适配器 │
│ (发送到平台)  │
└──────┬───────┘
       │
       ▼
    用户收到回复
```

---

## 🔑 核心概念详解

### 1. MessageBus（消息总线）
**文件**: `nanobot/bus/queue.py`

**作用**: 解耦聊天渠道和代理核心
- **Inbound Queue**: 从渠道到代理的消息
- **Outbound Queue**: 从代理到渠道的响应

**关键方法**:
- `publish_inbound(msg)` - 发布入站消息
- `consume_inbound()` - 消费入站消息
- `publish_outbound(msg)` - 发布出站消息
- `consume_outbound()` - 消费出站消息

### 2. AgentLoop（代理循环）
**文件**: `nanobot/agent/loop.py` (459 行)

**核心职责**:
1. 从 MessageBus 消费消息
2. 管理会话 (Session)
3. 构建上下文 (ContextBuilder)
4. 调用 LLM
5. 执行工具 (ToolRegistry)
6. 返回响应

**关键方法**:
- `run()` - 主循环
- `_process_message()` - 处理单条消息
- `_run_agent_loop()` - LLM + 工具调用循环

**详细文档**: [agentloop_analysis.md](./agentloop_analysis.md)

### 3. ContextBuilder（上下文构建器）
**文件**: `nanobot/agent/context.py` (253 行)

**作用**: 构建 LLM 的完整提示词

**组件**:
- 系统提示词
- 历史对话
- 技能 (Skills)
- 当前消息

### 4. ToolRegistry（工具注册表）
**文件**: `nanobot/agent/tools/registry.py`

**内置工具**:
- `ReadFileTool` - 读取文件
- `WriteFileTool` - 写入文件
- `EditFileTool` - 编辑文件
- `ExecTool` - 执行 shell 命令
- `WebSearchTool` - 网页搜索
- `WebFetchTool` - 网页抓取
- `MessageTool` - 发送消息
- `SpawnTool` - 启动子代理
- `CronTool` - 定时任务
- MCP 工具 - 外部工具集成

### 5. Session（会话管理）
**文件**: `nanobot/session/manager.py`

**作用**: 管理对话历史和状态

**关键特性**:
- 持久化存储
- 消息窗口管理
- 自动归档 (consolidation)

### 6. Memory（记忆系统）
**文件**: `nanobot/agent/memory.py` (150 行)

**作用**: 长期记忆存储和检索

### 7. Channels（渠道适配器）
**目录**: `nanobot/channels/`

**支持平台**:
- Telegram
- Discord
- WhatsApp
- Feishu (飞书)
- Slack
- Email
- QQ
- DingTalk (钉钉)
- Mochat

**统一接口**:
- 每个渠道实现 `Channel` 基类
- 将平台特定消息转换为 `InboundMessage`
- 将 `OutboundMessage` 转换为平台特定格式

### 8. Providers（LLM 提供商）
**目录**: `nanobot/providers/`

**核心文件**:
- `registry.py` - 提供商注册表（单一事实来源）
- `base.py` - 提供商基类
- `litellm_provider.py` - LiteLLM 提供商
- `custom_provider.py` - 自定义提供商

**支持的提供商**:
- OpenRouter (推荐)
- Anthropic (Claude)
- OpenAI (GPT)
- DeepSeek
- Groq
- Gemini
- MiniMax
- 等等...

---

## 🎯 学习路径建议

### 阶段 1: 理解架构（当前阶段）✅

**目标**: 理解整体架构和数据流

**已完成**:
- ✅ 查看项目结构
- ✅ 理解核心模块
- ✅ 理解数据流
- ✅ 深入 AgentLoop

**下一步**: 深入学习各个核心模块

### 阶段 2: 核心模块深入

**顺序建议**:
1. ✅ **AgentLoop** (`agent/loop.py`) - 主处理逻辑
2. **ContextBuilder** (`agent/context.py`) - 提示词构建
3. **ToolRegistry** (`agent/tools/`) - 工具系统
4. **Session管理** (`session/manager.py`) - 会话管理
5. **Memory系统** (`agent/memory.py`) - 记忆系统
6. **Channel适配器** (`channels/`) - 渠道集成
7. **Provider系统** (`providers/`) - LLM 集成

### 阶段 3: 关键功能点

**重要功能**:
- 工具调用机制
- 会话管理
- 记忆归档 (consolidation)
- 子代理 (subagent)
- MCP 集成
- 定时任务 (cron)
- 心跳机制 (heartbeat)

### 阶段 4: 实践和扩展

**建议练习**:
1. 添加一个新的工具
2. 添加一个新的渠道
3. 添加一个新的 LLM 提供商
4. 修改系统提示词
5. 实现自定义技能

---

## 📝 代码质量特点

### 设计原则
1. **单一职责**: 每个模块职责清晰
2. **松耦合**: 通过 MessageBus 解耦
3. **可扩展**: 易于添加新渠道、新工具、新提供商
4. **异步优先**: 全面使用 asyncio

### 代码风格
- 类型注解 (Type Hints)
- 清晰的函数和变量命名
- 适度的注释
- 模块化设计

---

## 🔍 关键代码片段索引

### 消息处理主循环
**位置**: `agent/loop.py:240-269`
```python
async def run(self) -> None:
    """Run the agent loop, processing messages from the bus."""
    while self._running:
        msg = await self.bus.consume_inbound()
        response = await self._process_message(msg)
        if response:
            await self.bus.publish_outbound(response)
```

### LLM + 工具调用循环
**位置**: `agent/loop.py:174-238`
```python
async def _run_agent_loop(self, initial_messages, on_progress=None):
    while iteration < self.max_iterations:
        response = await self.provider.chat(...)
        if response.has_tool_calls:
            # 执行工具
            result = await self.tools.execute(...)
        else:
            break
    return final_content
```

### 工具执行
**位置**: `agent/tools/registry.py`
- 工具注册
- 工具执行
- 参数验证

### 渠道适配器
**位置**: `channels/base.py`
- 基类定义
- 消息转换
- 生命周期管理

---

## 📚 学习文档索引

| 模块 | 文档 | 状态 |
|------|------|------|
| 整体架构 | [learning_guide.md](./learning_guide.md) | ✅ 已完成 |
| AgentLoop | [agentloop_analysis.md](./agentloop_analysis.md) | ✅ 已完成 |
| ContextBuilder | `context_builder_analysis.md` | 📝 待创建 |
| ToolRegistry | `tool_registry_analysis.md` | 📝 待创建 |
| Session管理 | `session_manager_analysis.md` | 📝 待创建 |
| Memory系统 | `memory_system_analysis.md` | 📝 待创建 |
| Channel适配器 | `channel_adapter_analysis.md` | 📝 待创建 |
| Provider系统 | `provider_system_analysis.md` | 📝 待创建 |

---

## 🚀 下一步学习建议

根据你的学习目标，可以选择以下路径：

### 路径 A: 按模块深入学习（推荐）
1. ContextBuilder - 理解如何构建 LLM 提示词
2. ToolRegistry - 学习工具系统的设计和实现
3. Session管理 - 掌握会话持久化机制
4. Memory系统 - 理解长期记忆和归档

### 路径 B: 跟踪数据流
1. 从用户输入到 LLM 响应的完整流程
2. 工具调用的详细机制
3. 会话状态管理

### 路径 C: 实践导向
1. 添加自定义工具
2. 集成新的聊天平台
3. 实现新的 LLM 提供商

---

**你想从哪个模块继续学习？**
